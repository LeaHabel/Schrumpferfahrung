...
namespace shrink.project.de
{
    ...
    public class ShrinkOrGrow : MonoBehaviour
    {
            ...
            switch (action)
            {
                    ...
                    switch (state)
                    {
                        case State.SHRUNKEN:
                            transform.localScale = new Vector3(
                                (1.0f - progress) * shrunkScale.x + progress * originalScale.x,
                                (1.0f - progress) * shrunkScale.y + progress * originalScale.y,
                                (1.0f - progress) * shrunkScale.z + progress * originalScale.z
                            );
                            break;
                        case State.GROWN:
                            transform.localScale = new Vector3(
                                (1.0f - progress) * grownScale.x + progress * originalScale.x,
                                (1.0f - progress) * grownScale.y + progress * originalScale.y,
                                (1.0f - progress) * grownScale.z + progress * originalScale.z
                            );
                            break;
                        default:
                            break;
                ...
                case Actions.GROWING:
                    switch (state)
                    {
                        case State.SHRUNKEN:
                            transform.localScale = new Vector3(
                                (1.0f - progress) * shrunkScale.x + progress * grownScale.x,
                        ...
                        case State.ORIGINAL_SIZE:
                            transform.localScale = new Vector3(
                                (1.0f - progress) * originalScale.x + progress * grownScale.x,
                                (1.0f - progress) * originalScale.y + progress * grownScale.y,
                                (1.0f - progress) * originalScale.z + progress * grownScale.z
                            );
                            break;
                        default:
                            break;
                ...
                case Actions.SHRINKING:
                    switch (state)
                    {
                        case State.ORIGINAL_SIZE:
                            transform.localScale = new Vector3(
                                (1.0f - progress) * originalScale.x + progress * shrunkScale.x,
                                (1.0f - progress) * originalScale.y + progress * shrunkScale.y,
                                (1.0f - progress) * originalScale.z + progress * shrunkScale.z
                            );
                            break;
                        case State.GROWN:
                            transform.localScale = new Vector3(
                                (1.0f - progress) * grownScale.x + progress * shrunkScale.x,
                                (1.0f - progress) * grownScale.y + progress * shrunkScale.y,
                                (1.0f - progress) * grownScale.z + progress * shrunkScale.z
                            );
                            break;
                        default:
                            break;
        ...
        public void shrinkCallback(){
            GetComponent<CapsuleCollider>().radius = 25.0f;
        }
    }
}